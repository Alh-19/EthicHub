import { QUERY_ETHIX_HOLDERS_CURRENT, QUERY_ETHIX_HOLDERS_CURRENT_ZERO } from '@/utils/queries'
import {
  getDayTimestampsSinceEthixMintEth,
  getDayTimestampsSinceEthixMintCelo,
  getTimestampToday,
...
export default {
  async getHoldersCurrent({ commit }, minAmount) {
    ...
    const query = minAmount === 0 ? QUERY_ETHIX_HOLDERS_CURRENT_ZERO : QUERY_ETHIX_HOLDERS_CURRENT
    ...
    commit('SET_HOLDERS_CURRENT', ['eth', dataEth.ethixHolders])
    commit('SET_HOLDERS_CURRENT', ['celo', dataCelo.ethixHolders])
  ...
  async getHoldersCompare({ commit }, { minAmount, timestamps }) {
    ...
    const queryNewEthixHoldersNoRecurrentEth = `
      query NewEthixHoldersNoRecurrent($dateJoinedFirst: BigInt!, $minAmount: BigDecimal!) {
        ethixHolders(
    ...
    const queryNewEthixHoldersNoRecurrentCelo = `
      query NewEthixHoldersNoRecurrent($dateJoinedFirst: BigInt!, $minAmount: BigDecimal!) {
        ethixHolders(
          first: 1000, orderBy: totalAmount, orderDirection: desc,
          where: {
    ...
    const { data: joinedNewEth } = await clientEth.query({
      query: gql(queryNewEthixHoldersNoRecurrentEth),
      variables: {
        minAmount: minAmount,
    ...
    const { data: joinedNewCelo } = await clientCelo.query({
      query: gql(queryNewEthixHoldersNoRecurrentCelo),
      variables: {
        minAmount: minAmount,
        dateJoinedFirst: timestamps[0] + 1,
      },
    ...
    commit('SET_HOLDERS_COMPARE_JOINED_NEW', ['eth', joinedNewEth.ethixHolders])
    commit('SET_HOLDERS_COMPARE_JOINED_NEW', ['celo', joinedNewCelo.ethixHolders])
  ...
  async getHoldersDayCountCelo({ dispatch }, minAmount) {
    ...
    const timestamps = getDayTimestampsSinceEthixMintCelo()
    const data = await dispatch('getHoldersDayCountHelper', {
      minAmount,
      timestamps,
      client: clientCelo,
